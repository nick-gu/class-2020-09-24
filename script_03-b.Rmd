...
# Combines through inner_join to only keep shared rows
# Filters ony for classes with > 10 undergrads
# Mutates to create new difference column
...
# Sorts and slices the top 5 smallest differences (largest abs of negative)

largedrop <- sep_diff %>% 
  arrange(differenceen) %>% 
  slice(1:5) %>%
  select(-(title.old:department.old))

# Sorts and slices the top 5 largest differences
...
# Binds the columns for the largest inc and dec

course_10 <- rbind(largedrop, largeinc)

# Uses gt() to make the table

largedrop %>% 
  gt() %>%
  tab_header(title = "Largest Differences in Enrollment") %>%
  cols_label(id = "ID", title.new = "Title", name.new = "Name", department.new = "Department", u_grad.new = "New Undergrad Enrollment", u_grad.old = "Old Undergrade Enrollment", differenceen = "Difference in Enrollment")

course_10 %>% ggplot(mapping = aes(x = title.new, y = differenceen, fill = department.new)) +
  geom_col() +
...
- Our ideal preceptor table would tell us every class's work assigned per day, the amount of time it took each student to complete, and non-measurable items like the psychological strain the work had on students.
- Much of this dat is missing, however. Many classes don't explicitly survey students to see the amount of hours each class took for every student and mental health 
